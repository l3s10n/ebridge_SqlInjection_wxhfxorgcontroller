...
wxhfxorgcontroller存在sql注入漏洞，在登录泛微云桥之后，可以借助该漏洞执行sql注入语句。
...
wxhfxorgcontroller.exportOrg函数调用了wxHfxOrgInfoService.exportOrg函数，该函数关键代码如下：
...
wxhfxorgcontroller.exportOrg调用wxHfxOrgInfoService.exportOrg函数的方式是这样的：
...
public void exportOrg() {
    try {
        ...
        if (StrKit.isBlank(syscorpid)) {
            ...
            this.renderFile(this.wxHfxOrgInfoService.exportOrg(syscorpid, depts, users, fields, syncond));
        }
